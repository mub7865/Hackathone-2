...
// Use relative URL if NEXT_PUBLIC_API_URL is empty (goes through middleware rewrite)
 ...
 * Uses Better Auth if feature flag enabled, otherwise legacy backend API
 */
export async function signIn(email: string, password: string): Promise<Session> {
  // Check if Better Auth is enabled
  const useNewAuth = process.env.NEXT_PUBLIC_FEATURE_NEW_AUTH === 'true';

  if (useNewAuth) {
    // Use Better Auth API route
